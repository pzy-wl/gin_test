...
func fileTest() {
	key := "0001112223334445"
	fmt.Println("length of key is:", len(key))
	dat, err := ioutil.ReadFile("../数据模板.xlsx")
	if err != nil {
	...
	ciphertext := encrypt(string1, key)
	fmt.Println("length of ciphertext is:", len(ciphertext))
	//fmt.Printf("after encrypt:[%s]\n", ciphertext)

	string3 := decrypt(ciphertext, key)
	//fmt.Printf("after decrypt:[%s]\n", string3)
	byte := []byte(string3)
	file, err := os.Create("moduel.xlsx")
	if err != nil {
	...
	if err != nil {
		fmt.Println("file write fail", err.Error())
		return
	}
}
